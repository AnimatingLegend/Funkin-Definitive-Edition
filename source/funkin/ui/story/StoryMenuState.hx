...
import funkin.ui.story.components.StoryMenuItem;
import funkin.ui.story.components.MenuCharacter;
...
class StoryMenuState extends MusicBeatState
{
	 ...
	 */
	private var weekDatas:Array<LevelData> = [];
	...
	function loadWeekData():Void
	{
		...
		for (fileName in WEEK_FILES)
		{
			...
			try
			{
				var data:LevelData = haxe.Json.parse(raw);
				weekDatas.push(data);
			}
			catch (err)
			{
	...
	{
		var data:LevelData = weekDatas[currentWeek];
